...
export default function PopupAddCenter({
    open,onClose,setPage,
    projectName, staffName, techName,
    getData, 
    url,
    title = "Thêm trung tâm, bộ phân, phòng ban ",
}) {
  ...
  const [techNameCenter, setTechNameCenter] = React.useState([]);
  const handleChangeTech = (event) => {
    setTechNameCenter(event.target.value);
  };
  const [projectNameCenter, setProjectNameCenter] = React.useState([]);
  const handleChangeProject = (event) => {
    setProjectNameCenter(event.target.value);
  };
  const [staffNameCenter, setStaffNameCenter] = React.useState([]);
